...
| Express JSON | 100 MB |
| Host Nginx `client_max_body_size` | **`UPLOAD_MAX_SIZE`** (default **`20G`**; `0` = unlimited) |
| Multer reel upload | **`UPLOAD_MAX_BYTES`** (default **`21474836480`** = 20GiB) |
...
`deploy.sh` hard-resets every clone to `origin/${DEPLOY_BRANCH:-main}` (`fetch` → `checkout` → `reset --hard` → `clean -fd`). It never uses `git pull`.
